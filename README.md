...
| `bridge_network_airgap`             | If the `bridge_network_airgap` is set to true, Synopsys Action will not download the Synopsys Bridge but instead uses the pre-configured Synopsys Bridge. If the Synopsys Bridge is configured at a specific location, provide the path through `synopsys_bridge_install_directory`, <br/><br/>The Synopsys Action will look for the Synopsys Bridge from `synopsys_bridge_install_directory` path otherwise it will look for the Synopsys Bridge in the default path ($HOME/synopsys-bridge) |
