...
| `network_air_gap`                   | If the `network_air_gap`  is set to true, Synopsys Action will not download the bridge but instead use the pre-configured bridge. If the bridge is configured at a specific location, provide the path through`synopsys_bridge_install_directory`. The Synopsys Action will look for the Synopsys Bridge from `synopsys_bridge_install_directory` path; otherwise, it will look for the Synopsys Bridge in the default path. |
