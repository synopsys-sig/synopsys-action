...
test('should fetch version successfully', async () => {
...
test('should throw an error if the HTTP status is not OK', async () => {
  const githubServerUrl = 'https://example.com'
  const incomingMessage: IncomingMessage = new IncomingMessage(new Socket())
  const httpResponse: Mocked<HttpClientResponse> = {
    message: incomingMessage,
...
test('should throw an error if fetching version fails', async () => {
  const githubServerUrl = 'https://example.com'
...
test('getGitHubClientServiceInstance() should return GithubClientServiceCloud service', async () => {
  process.env['GITHUB_SERVER_URL'] = 'https://github.com'

  expect(await GitHubClientServiceFactory.getGitHubClientServiceInstance()).toBeInstanceOf(GithubClientServiceCloud)
})

test('getGitHubClientServiceInstance() should return GithubClientServiceV1 service for version 3.12', async () => {
...
test('getGitHubClientServiceInstance() should return GithubClientServiceV2 service for version 3.13', async () => {
  process.env['GITHUB_SERVER_URL'] = 'https://example.com'
...
test('getGitHubClientServiceInstance() should return GithubClientServiceV2 service for unknown version', async () => {
  process.env['GITHUB_SERVER_URL'] = 'https://example.com'
