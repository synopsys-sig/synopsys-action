  ...
  try {
    ...
    if (isBridgeExecuted) {
      ...
      if (!isPullRequestEvent() && uploadSarifReportBasedOnExitCode) {
        ...
        if (inputs.BLACKDUCK_URL && parseToBoolean(inputs.BLACKDUCK_REPORTS_SARIF_CREATE) && inputs.BLACKDUCK_WAITFORSCAN !== 'false') {
        ...
        if (inputs.POLARIS_SERVER_URL && parseToBoolean(inputs.POLARIS_REPORTS_SARIF_CREATE) && inputs.POLARIS_WAITFORSCAN !== 'false') {
        ...
        if (!isNullOrEmptyValue(inputs.GITHUB_TOKEN)) {
          ...
          if (inputs.BLACKDUCK_URL && parseToBoolean(inputs.BLACKDUCK_UPLOAD_SARIF_REPORT) && inputs.BLACKDUCK_WAITFORSCAN !== 'false') {
          ...
          if (inputs.POLARIS_SERVER_URL && parseToBoolean(inputs.POLARIS_UPLOAD_SARIF_REPORT) && inputs.POLARIS_WAITFORSCAN !== 'false') {
            const gitHubClientService = await GitHubClientServiceFactory.getGitHubClientServiceInstance()
